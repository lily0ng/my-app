...
export function DocsShell({
  ...
  const topItems = [
    ...
    },
    {
      ...
      active: pathname.startsWith('/docs/guides/api') || (pathname.startsWith('/docs/guides') && !pathname.startsWith('/docs/guides/product') && !pathname.startsWith('/docs/guides/help-center') && !pathname.startsWith('/docs/guides/changelog')),
    },
    { label: 'Help Center', href: '/docs/guides/help-center/', active: pathname.startsWith('/docs/guides/help-center') },
    { label: 'Changelog', href: '/docs/guides/changelog/', active: pathname.startsWith('/docs/guides/changelog') },
  ];
  ...
  return (
    <div className="min-h-screen w-full bg-[color:var(--bg-primary)] text-[color:var(--text-primary)]">
        ...
        <div className="mx-auto w-full max-w-[1280px] px-6">
          <div className="h-16 flex items-center gap-8">
            <Link to="/" className="flex items-center gap-2">
              <img src={logo} alt="Logo" className="h-8 w-auto" loading="eager" />
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-10">
              {topItems.map((it) => (
                <Link
                  ...
                  to={it.href}
                  className={
                    'text-lg font-semibold transition-colors relative pb-5 -mb-5 ' +
                    (it.active
                ...
                >
                  {it.label}
                  {it.active && <span className="absolute left-0 right-0 -bottom-[1px] h-[3px] bg-[color:var(--accent)]" />}
                </Link>
              ))}
            </nav>
              ...
              {hasSearchHandler && (
                <div className="hidden lg:block relative w-[420px]">
                  ...
                  <input
                    ...
                    className="w-full bg-[color:var(--bg-secondary)] border border-[color:var(--border-color)] rounded-lg py-2 pl-9 pr-3 text-sm text-[color:var(--text-primary)] focus:outline-none focus:border-[color:var(--accent)] transition-colors"
                  />
                </div>
              )}
            </div>
      ...
      <div className="sticky top-16 z-40 bg-[color:var(--bg-primary)] border-b border-[color:var(--border-color)]">
        <div className="mx-auto w-full max-w-[1280px] px-6">
          <div className="h-11 flex items-center gap-3">
            <button
              type="button"
              onClick={onToggleSidebar}
              className="inline-flex items-center gap-2 rounded-md border border-[color:var(--border-color)] bg-[color:var(--bg-secondary)] px-4 py-2 text-sm font-semibold text-[color:var(--text-primary)] hover:border-[color:var(--accent)] transition-colors"
            >
      ...
      <div className="mx-auto w-full max-w-[1280px] px-6 pt-6">
        <div
          className={
            ...
            (sidebarOpen
              ? 'lg:grid-cols-[320px_minmax(0,760px)_240px]'
